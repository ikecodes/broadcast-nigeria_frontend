import { FETCH_PRODUCTS, ADD_TO_CART, CLEAR_CART, GET_TOTAL_PRICE, INCREASE_PRODUCT, DECREASE_PRODUCT } from '../constants/actionTypes';
...
export const getProducts = () => async (dispatch) => {
  try {
    ...
    const res = data.reduce((acc, cur) => {
      acc.push({ id: cur.id, name: cur.name, price: cur.price.raw, image: cur.media.source, number: 1 });
      return acc;
    }, []);
...
export const addToCart = (id) => (dispatch) => dispatch({ type: ADD_TO_CART, payload: id });
